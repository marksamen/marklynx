...
document.addEventListener("keydown",e=>{
  ...
  if(document.getElementById("gameDevRecoveryModal").classList.contains("open"))return;
...
let recoveryJsonPromptResolve=null;
function closeRecoveryJsonPrompt(download){
  const modal=document.getElementById("gameDevRecoveryModal");
  if(!modal.classList.contains("open"))return;
  modal.classList.remove("open");
  modal.setAttribute("aria-hidden","true");
  const resolve=recoveryJsonPromptResolve;
  recoveryJsonPromptResolve=null;
  if(resolve)resolve(download);
}
function askRecoveryJsonDownload(){
  const modal=document.getElementById("gameDevRecoveryModal");
  modal.classList.add("open");
  modal.setAttribute("aria-hidden","false");
  return new Promise(resolve=>{recoveryJsonPromptResolve=resolve;});
}
async function offerRecoveryJsonDownload(){
  if(await askRecoveryJsonDownload()){
    document.getElementById("gameDevExportSupabase").click();
  }
}
document.getElementById("gameDevRecoveryYes").addEventListener("click",()=>closeRecoveryJsonPrompt(true));
document.getElementById("gameDevRecoveryNo").addEventListener("click",()=>closeRecoveryJsonPrompt(false));
