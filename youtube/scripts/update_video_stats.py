...
def main():
    ...
    for row in raw:
        if not isinstance(row, dict):
            continue
        # Current site data uses p for playlist. These aliases make the updater
        # tolerant of future naming without changing the website.
        for key in ("p", "playlist", "playlistId", "playlistUrl"):
            pid = playlist_id(row.get(key))
            if pid:
