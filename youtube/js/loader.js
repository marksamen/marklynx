(() => {
  ...
  (async () => {
    try {
      ...
      // Production: Firestore replaces games.json as the source of window.RAW.
      // Production collection: games.
      ...
      if (!firestoreResponse.ok) {
        throw new Error(`games: HTTP ${firestoreResponse.status}`);
      }
      ...
      console.info(`[PRODUCTION] Firestore games loaded: ${window.RAW.length} visible / ${allGames.length} total`);
