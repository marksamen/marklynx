  ...
  loadYouTubeIframeAPI().then(()=>{
    ...
    const runAttempt = (entry, index, attempt, originalStart)=>{
      ...
      const collect = (source)=>{
        ...
        try{
          const ids = scanner.getPlaylist() || [];
          if(ids.length){
            ids.forEach(id=>{ if(id) siteVideoIds.add(id); });
            updateTotalVideoCount();
            console.log(`[SCAN] ${label} COLLECT via ${source} | ${ids.length} videos`);
            finalizeAttempt('OK','',ids.length);
            return true;
          }
