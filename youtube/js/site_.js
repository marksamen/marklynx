...
// TEST REV19: Text Guide quick filter. Keep this with the existing Difficulty quick filters
// so it composes with Search, Genre, Platform and Sort without adding another control.
const quickFiltersGroup = diffFilter.querySelector('optgroup[label="Quick Filters"]');
if(quickFiltersGroup){
  const o=document.createElement('option');
  o.value='__TEXT_GUIDE__';
  o.textContent='📄 Text Guides Only';
  quickFiltersGroup.appendChild(o);
}
...
function getFiltered(){
  ...
  let out = RAW.filter(r=>{
    ...
    if(df === '__QE_ONLY__'){
      ...
      if(r.df === 'Quick & Easy') return false;
    } else if(df === '__TEXT_GUIDE__'){
      if(!hasTextGuide(r.tx)) return false;
    } else if(df){
