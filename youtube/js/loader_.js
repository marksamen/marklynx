(() => {
  ...
  (async () => {
    try {
      ...
      const baselineResponse = await fetch('data/games.json?v=20260917-panda', { cache: 'no-store' });
      if (!baselineResponse.ok) throw new Error(`games.json baseline: HTTP ${baselineResponse.status}`);
      const baselineGames = await baselineResponse.json();
