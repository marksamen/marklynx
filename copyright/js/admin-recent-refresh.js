const SUPABASE_RECENT_REFRESH_URL="https://aikifibkcjibubqegvmb.supabase.co";
...
export function initRecentRefresh({auth}){
  ...
  button.addEventListener("click",async()=>{
    ...
    try{
      ...
      const response=await fetch(`${SUPABASE_RECENT_REFRESH_URL}/functions/v1/recent-refresh`,{
        method:"POST",
        headers:{
      ...
      if(!response.ok)throw new Error(result.error||`Recent Refresh service returned HTTP ${response.status}.`);
